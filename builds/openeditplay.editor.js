(function (factory) {
    ...
    ***************************************************************************** */
    ...
    function assert(condition) {
        ...
        // @ifndef OPTIMIZE
        if (!condition) {
            console.warn.apply(console, __spreadArrays(['Assert'], messages, ['\norigin', changeGetter.get()]));
    ...
    function createValidator(name, validatorFunction) {
        var validator = function () {
            ...
            var parameters = args;
            var validatorArgs = __spreadArrays([null], args);
            return {
                validatorName: name,
                validate: function (x) {
                    validatorArgs[0] = x;
    ...
    */
    ...
    var Entity = /** @class */ (function (_super) {
        ...
        */
        Entity.prototype.addComponents = function (components, _a) {
            var _b = _a === void 0 ? {} : _a, _c = _b.fullInit, fullInit = _c === void 0 ? true : _c;
            assert(this._alive, ALIVE_ERROR);
            ...
            for (var i = 0; i < components.length; i++) {
                ...
                var componentList = this.components.get(component._name) || this.components.set(component._name, []).get(component._name);
    ...
    var Prototype = /** @class */ (function (_super) {
        ...
        function Prototype(predefinedId, siblingId) {
            ...
            _this.siblingId = siblingId || Prototype.createSiblingId();
            return _this;
        }
        Prototype.createSiblingId = function () {
        ...
        Prototype.prototype.cloneWithNewSiblingId = function () {
    ...
    var EntityPrototype = /** @class */ (function (_super) {
        ...
        EntityPrototype.prototype.cloneWithNewSiblingId = function () {
            var clone = this.clone();
            clone.siblingId = Prototype.createSiblingId();
            return clone;
        };
        ...
        prototype: {
            ...
            getConvexPoints: function (vectorClass, takeScaleIntoAccount) {
                ...
                for (var i = 0; i < this.points; ++i) {
                    var x = Math.sin(currentAngle) * this.radius;
                    var y = Math.cos(currentAngle) * this.radius;
    ...
    */
                ...
                if (this.trigger === 'playerComesNear') {
                    ...
                    for (var i = 0; i < entities_1.length; ++i) {
                        if (entities_1[i].Transform.getGlobalPosition().distanceSq(pos) < distSq) {
                            if (!entities_1[i][this.storeProp] && this.launchTrigger(entities_1[i]) !== false)
                                { break; }
                            entities_1[i][this.storeProp] = true;
                        }
                        else {
                            entities_1[i][this.storeProp] = false;
                        }
                    }
                }
            },
            ...
            onUpdate: function (dt, t) {
                ...
                for (var i = 0; i < this.particleCount; i++) {
                    ...
                    if (!p.alive) {
                        ...
                        if (t >= p.nextBirth) {
                            // The birth!
                            p.sprite = new PIXI$1.Sprite(this.texture);
    ...
    var AnimatorTrack = /** @class */ (function () {
        function AnimatorTrack(trackData, frames, rootEntityPrototype) {
            ...
            for (var _i = 0, keyFrameFrames_1 = keyFrameFrames; _i < keyFrameFrames_1.length; _i++) {
                ...
                var value = this.animatedProperty.propertyType.type.fromJSON(trackData.keyFrames[frame]);
                this.keyFrames.push({
                    frame: frame,
                    value: value,
                    control1: value,
            ...
            for (var i = 0; i < this.keyFrames.length; i++) {
                ...
                var curr = this.keyFrames[(i + 1) % this.keyFrames.length];
                var next = this.keyFrames[(i + 2) % this.keyFrames.length];
        ...
        ModuleContainer.prototype.activateOneOfModules = function (modules, unpackModuleView) {
            ...
            for (var i = 0; i < this.modules.length; ++i) {
                ...
                if (m._selected && modules.indexOf(m) >= 0) {
                    // Already selected
                    if (args.length > 0) {
                        this.activateModule.apply(this, __spreadArrays([m, unpackModuleView], args));
                    }
                    return;
                }
            }
            for (var i = 0; i < this.modules.length; ++i) {
                ...
                if (m._enabled && modules.indexOf(m) >= 0)
                    { return this.activateModule.apply(this, __spreadArrays([m, unpackModuleView], args)); }
            }
        };
    ...
    var Layout = /** @class */ (function () {
        function Layout() {
            ...
            var addContainer = function () {
                ...
                var container = new (ModuleContainer.bind.apply(ModuleContainer, __spreadArrays([void 0], args)))();
    ...
    var Module = /** @class */ (function () {
        ...
        */
        Module.activateModule = function (moduleId, unpackModuleView) {
            ...
            (_a = moduleIdToModule[moduleId].moduleContainer).activateModule.apply(_a, __spreadArrays([moduleIdToModule[moduleId], unpackModuleView], args));
        };
        /**
         * Modules must be in same moduleContainer
         * You might want to first call editor update to first enable the modules you want to activate.
        */
        Module.activateOneOfModules = function (moduleIds, unpackModuleView) {
            ...
            (_a = moduleIdToModule[moduleIds[0]].moduleContainer).activateOneOfModules.apply(_a, __spreadArrays([moduleIds.map(function (mId) { return moduleIdToModule[mId]; }), unpackModuleView], args));
        };
    ...
    var TopBarModule = /** @class */ (function (_super) {
        ...
        TopBarModule.prototype.addKeyboardShortcut = function (key, buttonOrCallback) {
            if (typeof buttonOrCallback === 'function') {
                this.keyboardShortcuts[key] = buttonOrCallback;
            }
            else {
                this.keyboardShortcuts[key] = function () { return buttonOrCallback.callback(buttonOrCallback.el); };
            }
        };
    ...
    function addEntitiesToLevel(entities) {
        ...
        var entityPrototypes = entities.map(function (entity) {
            ...
            var epr = entity.prototype.cloneWithNewSiblingId();
    ...
    var CreateObject = /** @class */ (function (_super) {
        ...
        function CreateObject() {
            ...
            var levelEntityPrototype = addEntitiesToLevel([entity]);
    ...
    var WidgetManager = /** @class */ (function () {
        function WidgetManager() {
            ...
            forEachScene(function (scene) {
                scene.listen(GameEvent.SCENE_DRAW, function () {
    ...
    */
    var SceneModule = /** @class */ (function (_super) {
        ...
        function SceneModule() {
            ...
            editorEventDispacher.listen(EditorEvent.EDITOR_CLONE, function () {
                ...
                if (editorSelection.type === 'epr') {
                    ...
                    (_a = _this.newEntities).push.apply(_a, entityPrototypes.map(function (epr) { return epr.cloneWithNewSiblingId().createEntity(epr.previouslyCreatedEntity.getParent()); }));
            ...
            globalEventDispatcher.listen('scene load level before entities', function (scene, level) {
                assert(!scene.layers.editorLayer, 'editorLayer should not be there');
                scene.layers.editorLayer = new PIXI$1.Container();
                scene.layers.move.addChild(scene.layers.editorLayer);
                scene.layers.widgetLayer = new PIXI$1.Container();
                scene.layers.positionHelperLayer = new PIXI$1.Container();
                scene.selectionLayer = new PIXI$1.Container();
                scene.layers.editorLayer.addChild(scene.layers.widgetLayer, scene.layers.positionHelperLayer, scene.selectionLayer);
            });
        ...
        SceneModule.prototype.entityClicked = function (entity, component) {
            ...
            if (this.selectedEntities.indexOf(entity) < 0) {
                // debugger;
                if (isMultiSelectModifierPressed()) {
                    this.selectEntities(__spreadArrays(this.selectedEntities, [entity]));
                }
                else {
    ...
    - Don't sync from scene to level. addChange enabled in scene edits only when properties of selected entities change. (for property editor)
        ...
        SceneModule.prototype.copyEntityPrototypes = function (entityPrototypes) {
            ...
            (_a = this.copiedEntityPrototypes).push.apply(_a, entityPrototypes.map(function (epr) { return epr.cloneWithNewSiblingId(); }));
        };
        SceneModule.prototype.pasteEntities = function () {
            var _a;
            this.deleteNewEntities();
            (_a = this.newEntities).push.apply(_a, this.copiedEntityPrototypes.map(function (epr) { return epr.cloneWithNewSiblingId().createEntity(); }));
    ...
    var TreeView = /** @class */ (function () {
        ...
        TreeView.prototype.createNode = function (id, text, parent) {
            $(this.el).jstree().create_node(parent || '#', { id: id, text: text }, 'last');
        };
            ...
            _this.treeView = new TreeView({
                ...
                selectionChangedCallback: function (selectedIds) {
                    if (_this.externalChange)
                        { return; }
                    var serializables = selectedIds.map(getSerializable).filter(Boolean);
                    if (serializables.length === 1 && editorSelection.items.length === 1 && serializables[0] === editorSelection.items[0]) ;
                    else {
                        selectInEditor(serializables, _this);
                    }
                    Module.activateModule('object', false);
                },
    ...
    editors.enum = function (container, oninput, onchange, options) {
        var select = el.apply(void 0, __spreadArrays(['select'], options.propertyType.validator.parameters.map(function (p) { return el('option', p); })));
    ...
    */
    ...
    var AnimationModule = /** @class */ (function (_super) {
        ...
        function AnimationModule() {
            ...
            redomListen(_this, 'animationSelected', function (animation) {
                _this.selectedAnimation = animation;
                ...
                _this.recordButton.style.display = animation ? 'inline-block' : 'none';
    ...
    var AnimationSelector = /** @class */ (function () {
        function AnimationSelector() {
            ...
            this.list = list(this.el, AnimationSelectorOption, (function (key) { return key; }));
        }
        AnimationSelector.prototype.update = function (animations) {
            this.animations = animations;
            this.list.update(__spreadArrays([null], animations.map(function (anim) { return anim.name; })));
        };
            ...
            if (['ent', 'epr', 'pfa', 'prt'].includes(editorSelection.type)) {
                editorEventDispacher.dispatchWithResults(EditorEvent.EDITOR_DELETE_CONFIRMATION).then(function (results) {
                    if (results.filter(function (res) { return res !== true; }).length === 0) {
                        ...
                        setChangeOrigin(editor);
                        serializables_1.forEach(function (s) { return s.delete(); });
        ...
        if (editorSelection.focused && editorSelection.items.length > 0) {
            if (['pfa', 'prt'].includes(editorSelection.type)) {
                ...
                filteredSerializabled.forEach(function (serializable) {
                    var parent = serializable.getParent();
                    var clone = serializable.cloneWithNewSiblingId();
                    if (parent) {
                        var _a = parseTextAndNumber(serializable.name), text = _a.text, number = _a.number;
                        var nameSuggestion_1 = text + number++;
                        while (parent.findChild(editorSelection.type, function (prt) { return prt.name === nameSuggestion_1; })) {
                            nameSuggestion_1 = text + number++;
                        }
