...
createPropertyType.float = createDataType({
	name: 'float',
	validators: {
...
createPropertyType.int = createDataType({
	name: 'int',
	validators: {
...
createPropertyType.vector = createDataType({
	name: 'vector',
	validators: {
...
createPropertyType.string = createDataType({
	name: 'string',
	validators: {
...
createPropertyType.bool = createDataType({
	name: 'bool',
	validators: {
...
createPropertyType.enum = createDataType({
	name: 'enum',
	validators: {
...
var Scene = (function (Serializable$$1) {
	...
	Scene.prototype.spawn = function spawn (prototype, position) {
		...
		var entity = prototype.createEntity(this);
...
editors.float = editors.int = function (container, oninput, onchange) {
	...
	mount(container, input);
	return function (val) { return input.value = +(+val).toFixed(4); };
};
...
function syncAChangeBetweenSceneAndLevel(change) {
	...
	if (change.type === changeType.addSerializableToTree) {
		...
		var property = ref;
		var cda = property.findParent('cda');
		if (!cda)
			{ return; }
		var prototype = cda.getParent();
		if (prototype.threeLetterType === 'epr') {
			...
			console.log('poi', scene.level.getChildren('epr').filter(function (epr) {
...
var EntityPrototype = (function (Prototype$$1) {
	...
	EntityPrototype.prototype.clone = function clone () {
		var clone = Prototype$$1.prototype.clone.call(this);
...
EntityPrototype.createFromPrototype = function(prototype, componentDatas) {
	...
	var transform = componentDatas.find(function (cda) { return cda.name === 'Transform'; });
	if (!transform) {
		transform = new ComponentData('Transform');
		componentDatas.push(transform);
	}
	
	var position = transform.findChild('prp', function (prp) { return prp.name === 'position'; });
	if (!position) {
...
var SceneModule = (function (Module$$1) {
	...
	SceneModule.prototype.clearEntityListeners = function clearEntityListeners () {
		this.entityListeners.forEach(function (listener) { return listener(); });
	...
	SceneModule.prototype.selectSelectedEntitiesInEditor = function selectSelectedEntitiesInEditor () {
		...
		var draw = function () { return this$1.draw(); };
		editor.select(this.selectedEntities, this);
		this.selectedEntities.forEach(function (ent) {
			this$1.entityListeners.push(ent.listen('changedInEditor', draw));
		});
	};
