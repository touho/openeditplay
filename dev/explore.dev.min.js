...
Serializable.prototype.addChild = function addChild (child) {
	...
	var parent = this;
	while (parent) {
		if (parent.threeLetterType === Serializable.rootThreeLetterType) {
...
Serializable.rootThreeLetterType = 'gam';
...
Serializable.registerSerializable(Component, 'com');
	...
	prototype: {
		...
		onUpdate: function onUpdate(dt, t) {
			if (this.userControlled) {
				...
				var change = new Victor(1, 0).rotate(t * this.speed).multiply(this.change);
...
var Prototype = (function (PropertyOwner$$1) {
	...
	Prototype.prototype.addChild = function addChild (child) {
		if (child.threeLetterType === 'cda')
			{ assert(this.findChild('cda', function (cda) { return cda.componentId === child.componentId; }) === null); }
...
function listen(view, type, handler) {
	var el = view.el || view;
	el.addEventListener(type, function (ref) {
...
var ModuleContainer = function ModuleContainer(moduleContainerName, packButtonIcon) {
	...
	if (packButtonIcon) {
		this.el.onclick = function () { return this$1.el.classList.contains('packed') && this$1.el.classList.remove('packed') || undefined; };
...
Module.registerTopButton('Bell Types', 'fa-bell-o', function () { 
	Module.unpackModuleContainer('bottom');
	Module.activateModule('types');
	Module.activateModule('type');
}, 1);
Module.registerTopButton('Cube Instances', 'fa-cubes', function () {
...
var PropertyEditor = function PropertyEditor(editor) {
	...
	events.listen('change', function (change) {
		if (change.type === 'editorSelection') {
			this$1.dirty = true;
		} else if (change.type === changeType.addSerializableToTree && change.parent === this$1.item) {
			this$1.dirty = true;
		}
	});
	...
	listen(this, 'propertyEditorSelect', function (items) {
		this$1.editor.select(items, this$1);
	});
};
...
Property$2.prototype.update = function update (property) {
	...
	var editor = editors$1[this.property.propertyType.type.name] || editors$1.default;
	this.setValue = editor(this.content, function (val) { return this$1.oninput(val); }, function (val) { return this$1.onchange(val); }, property.propertyType);
	this.setValue(this.property.value);
...
var Scene = (function (Serializable$$1) {
	...
	Scene.prototype.animFrame = function animFrame () {
		...
		window.requestAnimationFrame(function () { return this$1.animFrame(); });
	};
	Scene.prototype.spawn = function spawn (prototype, position) {
		...
		inheritedComponentDatas.forEach(function (d) {
			var component = new d.componentClass(null, entity, { scene: this$1 });
	...
	Scene.prototype.play = function play () {
		...
		this.deleteChildren();
		var prototype = this.getParent().forEachChild('prt', function (p) { return this$1.spawn(p); }, true);
...
var SceneModule = (function (Module$$1) {
	function SceneModule(editor) {
		...
		this.scene = editor.state.game.findChild('sce');
		if (!this.scene) {
			this.scene = new Scene();
			editor.state.game.addChild(this.scene);
		}
...
Module.registerTopButton('Play', 'fa-play', function () {
...
var Types = (function (Module$$1) {
	function Types() {
		...
		this.addButton.onclick = function () {
			...
			this$1.state.game.addChild(prototype);
			this$1.editor.select(prototype);
			setTimeout(function () {
	...
	Types.prototype.update = function update () {
		...
		if (!this.jstreeInited) {
			$(this.jstree).attr('id', 'types-jstree').on('changed.jstree', function (e, data) {
				...
				this$1.editor.select(data.selected.map(getSerializable), this$1);
